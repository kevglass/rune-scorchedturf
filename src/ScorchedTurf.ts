...
export class ScorchedTurf implements graphics.Game, ActionListener {
    ...
    get courseComplete(): boolean {
        return (this.game && this.players && !this.game.dynamics.find(b => b.data.playerId)) ? true : false;
    }
    ...
    runUpdate(game: GameState): void {
        if (this.currentCourseNumber !== game.courseNumber) {
            this.course = JSON.parse(JSON.stringify(courseInstances[game.courseNumber]));
            if (this.course) {
            ...
            this.currentCourseNumber = game.courseNumber;
        }

        if (this.course && this.game) {
            this.course.world.dynamicBodies = this.game.dynamics;
        }

        // TODO : Work out the trail
    ...
    gameUpdate(update: GameUpdate): void {
        ...
        if (this.course && this.game.dynamics.filter(b => b.data.playerId).map(b => b.data.playerId).filter(id => !this.game?.completed.includes(id)).length === 1) {
    ...
    mouseDown(x: number, y: number): void {
        ...
        if (this.currentWorld && physics.atRest(this.currentWorld)) {
            ...
            if (this.game && this.myTurn()) {
                const body = this.game.dynamics.find(b => b.data?.playerId === this.localPlayerId);
                if (body) {
    ...
    render(): void {
        ...
        if (this.atStart) {
            ...
            if (this.hasCourseMessage) {
                ...
                graphics.centerText("Waiting for Next Course", graphics.height() - 36, this.fontBig);
            }
        }
        ...
        if (this.game && this.course) {
            ...
            const body = this.game.dynamics.find(b => b.data.playerId === this.game?.whoseTurn);
            ...
            const atRest = physics.atRest(this.currentWorld);
            if (atRest) {
            ...
            if (this.powerDragging && this.power > 30 && atRest) {
                ...
                if (body) {
                    if (body.data.playerId !== this.localPlayerId) {
                        ...
                        if (this.power > 50) {
                            ...
                            for (let i=0;i<15;i++) {
                                ...
                                vy += (200 / 10) * step;
                                graphics.alpha(0.2);
                                graphics.drawImage(this.whiteCircle, x-6, y-6, 12, 12, "rgba(0,0,0,0.2)");
                                graphics.alpha(1);
                    ...
                    graphics.drawImage(this.arrow, 20 * (1 / scale), (-this.arrow.height / 2), this.arrow.width, this.arrow.height, "rgba(" + col.r + "," + col.g + "," + col.b + ", 0)");
                    graphics.pop();
                    graphics.alpha(1);
                }
            } else {
                if (this.showSpinner && this.game.nextTurnAt === 0 && this.physicsAtRestTime() > 500 && this.currentBody && this.currentBody.data.playerId === this.localPlayerId) {
                    ...
                    graphics.pop();
                    if (this.firstDrag) {
                        graphics.drawImage(this.hand, (1 - Math.sin((this.frame * 0.02) % Math.PI)) * -50, (1 - Math.sin((this.frame * 0.02) % Math.PI)) * 30);
                    }
                    graphics.pop();
                }
            }
        ...
        graphics.fillRect(-40, 0, boundsWidth + 80, boundsHeight + 80, "rgba(0,0,0,0.1)")
        ...
        if (this.showTitle && this.game) {
            ...
            if (this.showTitle) {
                ...
                graphics.centerText("Hole " + (this.game.courseNumber + 1), (graphics.height() / 2) - 40, this.fontBig, "#ddd");
